...
export function Footer() {
  ...
  const whatsappLink = `https://wa.me/${whatsappNumber.replace(/\+/g, '')}?text=Hello%20${encodeURIComponent(BRAND_NAME)}!%20I'd%20like%20to%20inquire%20about%20your%20fragrances`;
    ...
    <footer className="bg-luxury-black border-t border-white/5">
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 py-16">
          <div>
            <p className="font-display text-2xl text-luxury-cream mb-4">{BRAND_NAME}</p>
            ...
            <div className="flex gap-4 mt-6">
              {socialLinks.map((link) => (
                <a
                  ...
                  className="w-9 h-9 border border-white/10 flex items-center justify-center text-luxury-smoke hover:text-luxury-gold hover:border-luxury-gold/40 transition-colors"
                >
          ...
          <div>
            <p className="luxury-label mb-6">Concierge</p>
            <div className="space-y-3 text-sm text-luxury-smoke font-light">
              <p>
                55 Rue du Faubourg Saint-Honoré
                <br />
                Paris, 75008
              </p>
              <a href={`mailto:${BRAND_EMAIL}`} className="block hover:text-luxury-cream transition-colors">
                {BRAND_EMAIL}
              </a>
              <a href="tel:+33142685400" className="block hover:text-luxury-cream transition-colors">
                +33 (0)1 42 68 54 00
              </a>
              <p className="text-luxury-gold-muted text-xs uppercase tracking-wideish">By appointment only</p>
            </div>
          </div>
        </div>
